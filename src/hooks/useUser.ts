...
export const editProfileByAdmin = async (
  ...
  try {
    ...
    const error = e as AxiosError<ApiErrorResponse>;
    throw new Error(
      error.response?.data?.message || "Failed to editing user profile!"
    );
  }
};
...
export const editPasswordByAdmin = async (
  ...
  try {
    const response = await axiosInstance.patch(
      `/admin/users/${userId}/editPassword`,
      data
    );
    return response.data;
  } catch (e) {
    const error = e as AxiosError<ApiErrorResponse>;
    throw new Error(
      error.response?.data?.message || "Failed to editing user password!"
    );
  }
};
...
export const deleteUserByAdmin = async (userId: string) => {
  try {
    ...
    const error = e as AxiosError<ApiErrorResponse>;
    throw new Error(
      error.response?.data?.message || "Failed to deleting user!"
    );
  }
};
