...
//     if (video && canvas && SocketClient.isConnected()) {
//       const context = canvas.getContext("2d");
//       if (context) {
...
//         // Gunakan template literal untuk membuat nama event yang dinamis
//         const crowdFrameEvent = `io-crowd-frame-${areaById?.id}` as const;

//         SocketClient.getInstance().emit(crowdFrameEvent, frame);
//       }
//     }
//   };
...
//   useEffect(() => {
//     if (areaById?.id) {
//       SocketClient.initialize(areaById?.id);
...
//       const crowdResultEvent = `io-crowd-result-${areaById.id}` as const;

//       socketInstance.on(crowdResultEvent, onCrowdResult);
...
//           clearInterval(intervalRef.current);
//         }
//         socketInstance.off(crowdResultEvent, onCrowdResult);
