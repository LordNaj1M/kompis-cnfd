  ...
  const handleCreateArea = async (e: React.FormEvent) => {
    ...
    try {
        ...
        toast.promise(createPromise, {
            loading: {title: 'Create Area Process', description: 'Please wait while we create area.',},
            success: {title: 'Create Area Successful', description: 'You can now use new area.', duration: 1000, isClosable: true, onCloseComplete() {navigate(`/admin/crowd-configuration`)}},
            error: (error) => ({title: 'Create Area Failed', description: 'An error occurred during create area: ' + error, duration: 5000, isClosable: true}),
        });
    } catch (error) {
        toast({
